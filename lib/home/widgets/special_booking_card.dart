...
class SpecialBookingCard extends StatelessWidget {
  const SpecialBookingCard({
    Key? key,
    this.payment,
    this.onTap,
  }) : super(key: key);
  ...
  Widget buildTicketRoute() {
    return Text(
      "${payment?.specialBookInfo.org} - ${payment?.specialBookInfo.des}",
  ...
  Widget buildDateSchedule() {
    return Text(
      DateFormat("dd MMM yyyy HH:ss").format(payment!.specialBookInfo.departure),
      style: const TextStyle(
        fontSize: 10,
