...
import '../pages/tourism_booking_detail_page.dart';

class TourismHomeDashboardState with ChangeNotifier {
  ...
  void fetchHistories() async {
    if(!isLoaded) {
      histories = await BookingServices.getTourismTransactionHistories();
  ...
  detailClick(Transaction p) {
    Navigator.pushNamed(
        context!,
        TourismBookingDetailPage.routeName,
