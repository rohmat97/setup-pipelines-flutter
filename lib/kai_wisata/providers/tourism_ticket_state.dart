...
import 'package:kai_wisata/kai_wisata/pages/02tourism_result_page.dart';

import '../../data/models/station.dart';
import '../arguments/tourism_search_arguments.dart';

enum PickFrom { jkt, smg, yk }

enum PickTo { sby, smg, bd }

class TourismTicketState with ChangeNotifier {
  ...
  PickFrom from = PickFrom.jkt;
  PickTo to = PickTo.sby;
  ...
  TourismSearchArguments getSearchTicketArguments() {
    return TourismSearchArguments(
        fromStation, toStation, fromDate, toDate, isRoundTrip, skema);
  }
  ...
  onNextButton() {
    ...
    if (isValid) {
      Navigator.pushNamed(context!, TourismResultPage.routeName,
          arguments: getSearchTicketArguments());
      // return [false, ""];
    } else {
      List<String> message = [];
